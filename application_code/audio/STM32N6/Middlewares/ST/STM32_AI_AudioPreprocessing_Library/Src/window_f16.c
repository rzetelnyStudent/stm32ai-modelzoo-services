...
void cosine_sum_window_create_f16(float16_t *pDst, uint32_t len, float64_t a0, float64_t a1, float64_t a2);
...
int32_t Window_f16_Init(float16_t *pDst, uint32_t len, eWindowTypedef type)
{
  ...
  {
    case WINDOW_HANN:
      cosine_sum_window_create_f16(pDst, len, 0.5, 0.5, 0.0);
      break;
    case WINDOW_HAMMING:
      cosine_sum_window_create_f16(pDst, len, 0.54, 0.46, 0.0);
      break;
    case WINDOW_BLACKMAN:
      cosine_sum_window_create_f16(pDst, len, 0.42, 0.5, 0.08);
      break;
    default:
