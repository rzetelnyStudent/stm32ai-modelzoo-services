...
static __INLINE float MelScale(float mel_freq, eMelFormulaTypedef type);
static __INLINE float InverseMelScale(float mel_freq, eMelFormulaTypedef type);
...
void MelFilterbank(MelFilterTypeDef *M, float32_t *pSpectrCol, float32_t *pMelCol)
{
  ...
  for (uint32_t i = 0; i < n_mels; i++)
  {
    ...
    arm_dot_prod_f32(&pSpectrCol[start_idx], pCoefs,stop_idx + 1 - start_idx, &pMelCol[i]);
...
static __INLINE float MelScale(float freq, eMelFormulaTypedef type)
{
  if (type != MEL_HTK) {
