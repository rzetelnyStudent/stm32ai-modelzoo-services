...
void audio_is16oCplxFFTf32( const int16_t *pInSignal, AudioToFFTBatch_t *S, float32_t *pOut) {
  ...
  for (size_t frame = 0; frame < S->nb_frames; frame++ )
  {
    ...
    audio_is16of32_fft(pInSignal, pFFT, S->pScratch);
    ...
    if ( (E_SPECTROGRAM_CPLX_FFT_TRANSPOSE      == S->transpose)
      ...
      if (E_SPECTROGRAM_CPLX_FFT_TRANSPOSE == S->transpose) {
        ...
        pOut[2*frame + S->nb_frames * nFFT + 1] = 0.0f;
      }
    ...
    // No transpose
      ...
      if (E_SPECTROGRAM_CPLX_FFT == S->transpose) {
        ...
        pOut[frame * loop_step + 1 ]       = 0.0f;
        // Last
        pOut[frame * loop_step + nFFT + 0] = S->pScratch[1];
        pOut[frame * loop_step + nFFT + 1] = 0.0f;
      }
...
void audio_iCplxFFTf32oFFTx(const float32_t *pFFTData, AudioSpEExtraPre_t *S, void *pOut)
{
  ...
  for (size_t frame = 0; frame < S->nb_frames; frame++) {
    ...
    // No transpose
    {
      ...
      memcpy(&pScratch[2], &pFFTData[frame*loop_step + 2], 2 * (nFFT/2 - 1) * sizeof(float32_t));
    }
    ...
    switch (S->eType) {
      ...
      case SPECTRUM_TYPE_MAGNITUDE:
        SpectrogramColumn(pScratch, pScratch, nFFT, S->eType, &sumf32);
        break;
      default:
      break;
    }
    ...
    if (S->pNorm) {
        ...
        float32_t normFactor = (float32_t)S->pNorm->maxNormValue / (float32_t)S->pNorm->audioMax;
        arm_scale_f32(pScratch, normFactor, pScratch, nFFT/2 + 1);
      }
    }
    // Internal format is packed
    if (NULL == S->pQuant) {
    // output float32_t
      float32_t *pDataOut = (float32_t *)pOut;
      ...
      float32_t inv_scale = S->pQuant->output_Q_inv_scale;
 ...
 * @return     None
 */
void audio_iCplxFFTf32xf32os16(const float32_t *pFFTDataIn, const float32_t *pMask, AudioSpEPostProc_t *S, int16_t **pInOut)
{
    int16_t *pOut = *pInOut;
    FFTTypeDef_t *pFFT = S->pFFT;
    float32_t *pWeightsWindow = S->pWeightsWindow;
    float32_t *pScatchOut = S->pScratchOut;
    float32_t *pData;
    float32_t *pWindow = pFFT->pWindow;
    const float32_t *pFFTData = pFFTDataIn;
    ...
    for (size_t frame = 0; frame < S->nb_frames; frame++) {
        ...
        /******************************************/
        float32_t *pScratch = pFFT->pScratch;
        ...
        arm_rfft_fast_f32(pFFT->pRfft, pScratch, S->pScratch, 1);
        ...
        if (NULL != pWindow) {
            arm_mult_f32(pData, pWindow, pData, pFFT->sDin.FrameLen);
        }
        /* Previous data not impact by overlapping */
        for (size_t i = 0; i < pFFT->sDin.pad_left; i++) {
            float32_t convert;
            if ( 0.0f == S->pWeightsWindow[idxW]) {
                convert = 0.0f;
            } else {
                convert = pScatchOut[idxW] / S->pWeightsWindow[idxW];
                convert = convert * 32768.0f;
                convert += convert > 0.0f ? 0.5f : -0.5f;            }
            *pOut = (q15_t) (__SSAT((q31_t) (convert), 16));
            pScatchOut[idxW] = 0.0f;
            S->pWeightsWindow[idxW] = 0.0f;
            pOut++;
            idxW++;
    ...
    // Final ?
    #if 0
    arm_float_to_q15(&pScatchOut[idxW],pOut,S->pFFT->sDin.FrameLen-S->hop_length);
