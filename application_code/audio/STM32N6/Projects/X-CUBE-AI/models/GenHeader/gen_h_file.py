...
def gen_h_user_file(config):
    ...
    with open(os.path.join(path, "ai_model_config.h"), "wt") as f:
        ...
        if config.model.model_type == "STFTTCNN" :
            ...
            f.write(f'#define CTRL_X_CUBE_AI_SENSOR_ODR                ({config.preprocessing.target_rate}.0F)\n')
            f.write('#define CTRL_X_CUBE_AI_PREPROC                   (CTRL_AI_STFT)\n')
            f.write('#define CTRL_X_CUBE_AI_POSTPROC                  (CTRL_AI_ISTFT)\n')
            f.write(f'#define CTRL_X_CUBE_AI_SPECTROGRAM_HOP_LENGTH    ({config.feature_extraction.hop_length}U)\n')
            f.write(f'#define CTRL_X_CUBE_AI_SPECTROGRAM_NFFT          ({config.feature_extraction.n_fft}U) /* power of 2 closest to CTRL_X_CUBE_AI_WINDOW_LENGTH */ \n')
            f.write(f'#define CTRL_X_CUBE_AI_SPECTROGRAM_WINDOW_LENGTH ({config.feature_extraction.window_length}U) \n')
            f.write(f'#define CTRL_X_CUBE_AI_SPECTROGRAM_COL_NO_OVL    ({config.deployment.frames_per_patch}U) \n')
            f.write(f'#define CTRL_X_CUBE_AI_SPECTROGRAM_COL_OVL       ({config.deployment.overlap_frames}U) \n')
            f.write(f'#define CTRL_X_CUBE_AI_AUDIO_OUT_DB_THRESHOLD    ({config.deployment.output_threshold}.0F) \n')
